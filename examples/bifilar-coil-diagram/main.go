// bifilar-coil-diagram renders a diagram for the coil.
package main
...
var (
	width  = flag.Int("width", 500, "Image width")
	height = flag.Int("height", 100, "Image height")
	out    = flag.String("out", "bifilar-coil-diagram.png", "PNG output filename")
...
func main() {
	...
	for n := 0; n < 2*nlayers; n++ {
		...
		if n < len(labels) {
			...
			text, err := Text(tp.X, tp.Y, innerTS, innerTS, "TR/TL", textFont, &Center)
			check(err)
			if n == 1 {
				text.RenderToDC(dc, tp.X, tp.Y+5*innerTS, innerTS, 0)
			}
	...
	for innerTR := 0; innerTR < nlayers; innerTR++ {
		...
		for outerTR := 0; outerTR < nlayers; outerTR++ {
			...
			for i := 0; i < 2*nlayers; i++ {
				...
				if next == "" {
					log.Fatalf("innerConnection[%v] missing", last)
				}
				log.Printf("i=%v, next=%v", i, next)
				if seen[next] {
					break
				}
				seen[next] = true
...
func genMaps(total int, onAxis bool) (opposite, mirror map[int]int) {
	...
	if total == 1 {
		return map[int]int{0: 1, 1: 0}, map[int]int{0: 1, 1: 0}
	}
